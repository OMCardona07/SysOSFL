...
namespace SysOSFL.DAL
{
    public class JefeProyectDAL
    {
        public int AgregarJefe(JefeProject pJefe)
        {
            ...
            _conn.Open();
            IDbCommand comando = BDComun.ObtenerComandos(string.Format("INSERT INTO JefeProyect(Nombres,Apellidos,Dui," +
        ...
        public static int ModificarJefeProyect(JefeProject pJefe)
        {
            using (IDbConnection conn = BDComun.ObtenerConexion())
            {
                ...
                comando.Parameters.AddWithValue("@Id", pJefe.IdJefeProyect);
        ...
        public int EliminarJefeProyect(Int64 pIdJefeProyect)
        {
            IDbConnection _conn = BDComun.ObtenerConexion();
            _conn.Open();
            IDbCommand comando = BDComun.ObtenerComandos(string.Format("DELETE FROM JefeProyect WHERE Id = {0}", pIdJefeProyect)
                , _conn);
            int resultado = comando.ExecuteNonQuery();
            _conn.Close();
            return resultado;
        ...
        public JefeProject ObtenerJefePorId(Int64 _listaJefes)
        {
            ...
            {
                _conn.Open();
                IDbCommand commando = BDComun.ObtenerComandos(string.Format("select * from JefeProyect where Id={0}", _listaJefes), _conn);
                IDataReader reader = commando.ExecuteReader();
                while (reader.Read())
                {
